...
public class MessageServiceImpl implements IMessageService {
	...
	public boolean pushMessage(MsgList msg, String... objIds) {
		log.info("推送消息：【{}】，推送对象【{}】", JSON.toJSONStringWithDateFormat(msg, "yyyy-MM-dd HH:mm:ss"), objIds);
		if (StringUtil.isBlank(msg.getMsgId())) {
			msgListMapper.insert(msg);
		}
		try {
	...
	private void push(MsgList msg, String objId) throws APIConnectionException, APIRequestException {
		...
		for (String jpushId : jpushIdList) {
			PushPayload payload = JiguangPush.buildPushObject_all_alias_alert(jpushId,
					JSON.toJSONStringWithDateFormat(msg, "yyyy-MM-dd HH:mm:ss"));
			jpushClient.sendPush(payload);
			log.info("推送消息成功：【"+ JSON.toJSONStringWithDateFormat(msg, "yyyy-MM-dd HH:mm:ss") +"】");
		}
	}
	...
	@Override
	public CommonResult findByTargetId(String targetId, Integer pageIndex, Integer pageSize) {
		if (pageIndex == null || pageSize == null) {
			return new CommonResult(ResultCode.PARAM_ERROR.getValue(), "分页参数为空！");
		}
		return new CommonResult(ResultCode.SUCC.getValue(), "查询成功！", msgListMapper.findByTargetId(targetId, pageIndex * pageSize, pageSize));
	}
