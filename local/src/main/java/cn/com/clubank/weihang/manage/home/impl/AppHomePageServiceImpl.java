...
public class AppHomePageServiceImpl implements AppHomePageService {
	...
	@Override
	public CommonResult appHomeData(String customerId) {
...
//		result.put("rcmtSection", buildRcmtSection()); //为你推荐 - 二期
	...
	private JSONObject buildActivitySection() {
		...
		// 约定好限时抢购排序码是001
		List<Map<String, Object>> lmtTimeBuyMaps = mallActivityGoodsMapper.selectBySequenceCode("001", 0, 5);
		for (Map<String, Object> map : lmtTimeBuyMaps) {
			//获取图片
		...
		List<Map<String, Object>> benefitsMaps = mallActivityGoodsMapper.selectBySequenceCode(null, 0, 2);
	...
	private JSONObject buildPointMallSection() {
		...
		List<CouponInventory> coupons = couponInventoryMapper.selectValidByType(4, 0, 2); //优惠券
		for (CouponInventory coupon : coupons) {
			discount.add(buildCouponJson(coupon.getCouponName(), coupon.getPicSmall(), coupon.getCrId(), ""));
		}
	 ...
	 * @return
	 */
	private JSONObject buildActivitySkuJson(String title, String imageUrl, String objId, String marketPrice, String price, String endTime, Integer productType) {
		JSONObject result = new JSONObject();
		result.put("title", title);
		result.put("imageUrl", imageUrl);
		result.put("objId", objId);
