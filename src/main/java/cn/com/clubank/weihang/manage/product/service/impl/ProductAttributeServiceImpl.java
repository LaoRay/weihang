...
public class ProductAttributeServiceImpl implements ProductAttributeService {
	...
	public CommonResult deleteAttrByAttrId(String attrId) {
		...
		if (attrValueList != null && attrValueList.size() > 0) {
			return CommonResult.result(ResultCode.FAIL.getValue(), "该属性下存在属性值，不允许删除");
		}
	...
	public String deleteAttrValue(String vId) {
		...
		ProductAttrValue productAttrValue = attrValueMapper.selectByPrimaryKey(vId);
		List<Map<String, String>> attrValueList = productValueMapper
				.selectAttrValueListByProductAttrId(productAttrValue.getAttrId());
		if (attrValueList != null && attrValueList.size() > 0) {
			json.put("apiStatus", ResultCode.FAIL.getValue());
			json.put("msg", "该属性值已有商品存在，不允许删除");
