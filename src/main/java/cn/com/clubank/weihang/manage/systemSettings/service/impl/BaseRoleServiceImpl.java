...
public class BaseRoleServiceImpl implements IBaseRoleService {
	...
	public String selectRoleListByNameAndPaged(String roleName, Integer pageIndex, Integer pageSize) {
		...
		List<BaseRole> list = baseRoleMapper.selectRoleListAndPaged(roleName, page.getStart(), page.getPageSize());
		page.setRows(total);
	...
	public String insertOrUpdateRole(BaseRole role) {
		...
		BaseRole info = baseRoleMapper.selectBySort(role.getSort());
		if (info != null && !info.getRoleId().equals(role.getRoleId())) {
			json.put("apiStatus", ResultCode.DATA_EXIST.getValue());
			json.put("msg", "标识码已经存在，请重新填写");
