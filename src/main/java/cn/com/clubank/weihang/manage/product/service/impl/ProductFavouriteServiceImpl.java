...
public class ProductFavouriteServiceImpl implements ProductFavouriteService {
	...
	public CommonResult collectionProduct(ProductFavourite productFavourite) {
		if (productFavourite == null || StringUtils.isBlank(productFavourite.getProductId())) {
	 ...
	 */
	@Override
	public CommonResult websiteFindCollectionList(String customerId, Integer pageIndex, Integer pageSize) {
		PageObject<Map<String, String>> page = new PageObject<>(pageIndex, pageSize);
		int total = favouriteMapper.selectFavouriteListSizeByCustomerId(customerId);
		page.setRows(total);
		List<Map<String, String>> list = favouriteMapper.selectListByCustomerId(customerId, page.getStart(),
				page.getPageSize());
