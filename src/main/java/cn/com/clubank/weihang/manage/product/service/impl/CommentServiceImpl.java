...
public class CommentServiceImpl implements CommentService {
	...
	public CommonResult findCommentListByProductId(String productId, Integer pageIndex, Integer pageSize) {
		...
		Integer totalComment = commentCount.containsKey("totalComment")
				? Integer.parseInt(commentCount.get("totalComment")) : 0;
		Integer totalScore = commentCount.containsKey("totalScore") ? Integer.parseInt(commentCount.get("totalScore"))
				: 0;
	 ...
	 */
	@Override
	public CommonResult findCommentByOrderDetailId(String orderDetailId) {
		if (StringUtils.isBlank(orderDetailId)) {
			return CommonResult.result(ResultCode.PARAM_ERROR.getValue(), "参数错误");
		}
		List<Comment> list = commentMapper.selectByOrderDetailId(orderDetailId);
	...
	public void automaticComment() {
		...
		if (list != null && list.size() > 0) {
			for (Map<String, String> map : list) {
				...
				comment.setAssessment(Comment.HIGH_COMMENT);
				try {
					log.info("base64编码结果为：" + Base64.encodeBase64String("该用户没有评论".getBytes("UTF-8")));
					comment.setContent(Base64.encodeBase64String("该用户没有评论".getBytes("UTF-8")));
	 ...
	 */
	@Override
	public CommonResult websiteFindCommentListByProductId(String productId, Integer pageIndex, Integer pageSize) {
		PageObject<Map<String, String>> page = new PageObject<>(pageIndex, pageSize);
		int total = commentMapper.selectCommentCount(productId);
		page.setRows(total);
		List<Map<String, String>> list = commentMapper.selectByProductId(productId, page.getStart(),
				page.getPageSize());
	 ...
	 */
	@Override
	public CommonResult websiteFindCommentCountByProductId(String productId) {
		// 获取评价统计信息
		Map<String, String> commentCount = commentMapper.selectCountAssmentByProductId(productId);
		Integer totalComment = commentCount.containsKey("totalComment")
				? Integer.parseInt(commentCount.get("totalComment")) : 0;
		...
		commentCount.put("highCommentRate",
				totalComment == 0 ? "0" : String.format("%.0f", (double) totalScore * 100 / (totalComment * 5)) + "%");
		return CommonResult.result(ResultCode.SUCC.getValue(), "查询成功", commentCount);
	}
}
