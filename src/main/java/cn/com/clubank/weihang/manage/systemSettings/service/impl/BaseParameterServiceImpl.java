...
public class BaseParameterServiceImpl implements BaseParameterService {
	...
	public String selectParameterListAndPaged(String name, String keyCode, Integer pageIndex, Integer pageSize) {
		...
		List<BaseParameters> list = parameterMapper.selectParameterListAndPaged(name, keyCode, page.getStart(),
				page.getPageSize());
		page.setRows(total);
	...
	@Override
	public String getValueByKeyCode(String keyCode) {
		BaseParameters info = parameterMapper.selectByKeyCode(keyCode);
		if (info == null) {
			return null;
		}
		return StringUtil.isNotBlank(info.getValue()) ? info.getValue() : info.getDefaultValue();
	}
}
