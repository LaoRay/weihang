...
public class WorkEvaluateServiceImpl implements IWorkEvaluateService {
	...
	public String getConsultantEvaluateList(Integer dutyType, String userId, Integer pageIndex, Integer pageSize) {
		...
		if (dutyType == BaseUser.DUTY_SERVICE_CONSULTANT) {
			...
			//洗车师
			list = workEvaluateMapper.selectByWashEngineerId(userId, PageObject.getStart(pageIndex, pageSize), pageSize);
			WorkEvaluateTotal = workEvaluateMapper.selectByWashEngineerIdTotal(userId);
		} else if (dutyType == BaseUser.DUTY_TECHNICIAN) {
			//技师
			list = workEvaluateMapper.selectByRepairEngineerId(userId, PageObject.getStart(pageIndex, pageSize), pageSize);
			WorkEvaluateTotal = workEvaluateMapper.selectByRepairEngineerIdTotal(userId);
		}
		...
		for (WorkEvaluate info : list) {
			...
			//查询车辆信息
			WorkPickupOrder wpo = workPickupOrderMapper.selectByPrimaryKey(info.getWpoId());
			Map<String,Object> car = carInfoMapper.selectCarDetailByCarId(wpo != null ? wpo.getCarId() : Constants.NULL);
			if (car != null) {
				evaluateInfo.put("carNo", car.containsKey("carNo") ? car.get("carNo").toString() : Constants.NULL);
				evaluateInfo.put("level", car.containsKey("level") ? car.get("level").toString() : Constants.NULL);
			} else {
