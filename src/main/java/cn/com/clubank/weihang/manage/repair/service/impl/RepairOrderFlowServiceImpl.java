...
public class RepairOrderFlowServiceImpl implements IRepairOrderFlowService {
	...
	public String inService(String repairId) {
		log.info("车间组长开始（返工）维修，维修单：{}", repairId);
		JSONObject json = new JSONObject();
		WorkRepair info = workRepairMapper.selectByPrimaryKey(repairId);
		if (info == null) {
			json.put("apiStatus", ResultCode.DATA_NOEXIST.getValue());
		...
		//推送消息给技师
		title = String.format("维修单号：%s", info.getWorkRepairNo());
		content = String.format("【%s】 维修工作已开始，请您开始维修！", info.getCarNo());
		iMessageService.pushMessage(new MsgList(MsgList.MSGTYPE_REPAIR, title, content, repairId),
	...
	public String result(String repairId, Integer type, String opinion) {
		...
		if (type == 0) {
			...
			workRepairMapper.updateByPrimaryKey(info);
			// 追加子订单-接车单
			WorkPickupOrder childWpo = new WorkPickupOrder(); //接车单
			childWpo.setWpoNo(baseCodeRuleService.getCode(BaseCodeRule.ITEM_CODE_JC)); // 接车单号
			childWpo.setReservationOrderNo(info.getReservationOrderNo());
			childWpo.setReceiveBy(info.getConsultantBy());// 接车人员
			childWpo.setCarId(info.getCarId());// 车辆id
			childWpo.setCarNo(info.getCarNo());
			childWpo.setCustomerId(info.getCustomerId());// 客户id
			childWpo.setFromType(info.getFromType());// 到店方式
			childWpo.setLinkman(info.getLinkman());// 联系人
			childWpo.setLinkmanMobile(info.getLinkmanMobile());// 联系人手机
			childWpo.setWorkType(2); 
			childWpo.setStatus(WorkPickupOrder.STATUS_NO_SERVE);
			childWpo.setConsultantBy(info.getConsultantBy());// 服务顾问
			childWpo.setOrderDetailIds(wpo.getOrderDetailIds()); // 所使用的商品:服务类、汽车用品类
			workPickupOrderMapper.insert(childWpo);
			// 追加子订单-维修单
			WorkRepair childOrder = new WorkRepair();
			childOrder.setWpoNo(childWpo.getWpoNo());
