...
public class WeihInterceptor implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		if (handler.getClass().isAssignableFrom(HandlerMethod.class)) {
			WeihAuthLog weihAuth = ((HandlerMethod) handler).getMethodAnnotation(WeihAuthLog.class);
			// 没有声明需要权限,或者声明不验证权限
			if (weihAuth == null || !weihAuth.validate()) {
				return true;
			} else {
				// 请求的token
				...
				if (StringUtils.isBlank(jsonString)) {
					...
					if (StringUtil.isBlank(token)) {
						log.info("您还未登录，请重新登录！");
						json.put("msg", "您还未登录，请重新登录！");
					} else {
